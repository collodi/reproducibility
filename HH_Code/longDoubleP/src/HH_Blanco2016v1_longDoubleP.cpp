...
void show_usage(string name)
{
	cerr << "Usage: " << name << " <option(s)> SOURCES"
			...
			<< "\t-pExcN\t<Persentage of excitatory neurons, long double ]0..1]>\n"
			<< endl;
}
		...
		if (string(argv[i]) == "-pExcN") {
			if (i + 1 < argc) { // Make sure we aren't at the end of argv!
				...
				if (pExcNeurons<=0 || pExcNeurons>1) {
					std::cerr << "-pExcN option requires long double argument ]0..1]." << std::endl;
					return -1;
				}
			} else { // Uh-oh, there was no argument to the destination option.
				std::cerr << "-pExcN option requires long double argument ]0..1]." << std::endl;
...
int main(int argc, char* argv[]) {
	...
	if (SAVE_SIMULATION) {
		...
		string fileNameStr = "./results/HH_BBT_case" +
				...
				to_string((int)(maxTimeSimulation/1000)) +"s_longDouble_IappInter.txt"; //"s_SValues72,88,86,24.txt";
		...
		fileNameStr = "./results/HH_BBT_case" +
				...
				to_string(nInhNeurons) + ",vI" +
				to_string((int)vInh) + ",t=" +
				to_string((int)(maxTimeSimulation/1000)) +"s_longDouble_IappInter,Epis.txt";
		...
		fileNameStr = "./results/HH_BBT_case" +
				...
				to_string(nInhNeurons) + ",vI" +
				to_string((int)vInh) + ",t=" +
				to_string((int)(maxTimeSimulation/1000)) +"s_longDouble_IappInter,Iapp.txt";
		...
		fileNameStr = "./results/HH_BBT_case" +
				...
				negPosVInh +
				to_string((int)(maxTimeSimulation/1000)) + "s_longDouble_IappInter_Spikes.m";
